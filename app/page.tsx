...
export default function Page() {
  ...
  const [ssoModalPlatform, setSsoModalPlatform] = useState<"Google" | "Facebook" | null>(null);

  // Simulate Social Authentication OAuth with spinner delay
  const handleSocialSSO = (platform: "Google" | "Facebook") => {
    setSsoModalPlatform(platform);
  };

  const confirmSocialSSO = (selectedEmail: string) => {
    if (!ssoModalPlatform) return;
    const platform = ssoModalPlatform;
    setSsoModalPlatform(null);
    setSsoLoading(platform);
    setAuthError("");
    setTimeout(() => {
      setSsoLoading(null);
      const usernameMatch = selectedEmail.split("@")[0];
      const username = usernameMatch ? usernameMatch.charAt(0).toUpperCase() + usernameMatch.slice(1) : `${platform}User`;
      handleLogin(selectedEmail.toLowerCase(), username);
    }, 1200); // realistic pleasant animation loader delay
  };
      ...
      <div className="flex min-h-screen bg-slate-900 selection:bg-indigo-600 selection:text-white">
        ...
        <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 relative bg-white overflow-hidden">
          ...
          <div className="w-full max-w-[400px] relative z-10 jrpg-box-anim">
            ...
            {/* Google & Facebook button simulation bar */}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-900">Pilih Akun</h3>
                  <p className="text-xs text-slate-500">Lanjutkan ke Novelpedia dengan {ssoModalPlatform}</p>
                </div>
              </div>
              
              <div className="flex flex-col gap-3">
                <button 
                  onClick={() => confirmSocialSSO(`irsyalfaiz97@gmail.com`)}
                  className="w-full flex items-center gap-3 p-3 rounded-2xl hover:bg-slate-50 border border-slate-100 transition-colors text-left"
                >
                  <div className="w-10 h-10 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center font-bold text-lg">
                    I
                  </div>
                  <div>
                    <p className="text-sm font-bold text-slate-900">Irsyal Faiz</p>
                    <p className="text-xs text-slate-500">irsyalfaiz97@gmail.com</p>
                  </div>
                </button>
                <button 
                  onClick={() => confirmSocialSSO(`yareyyaa0529@gmail.com`)}
                  className="w-full flex items-center gap-3 p-3 rounded-2xl hover:bg-slate-50 border border-slate-100 transition-colors text-left"
                >
                  <div className="w-10 h-10 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center font-bold text-lg">
                    Y
                  ...
                  <div>
                    <p className="text-sm font-bold text-slate-900">Admin 2</p>
                    <p className="text-xs text-slate-500">yareyyaa0529@gmail.com</p>
                  </div>
                </button>

                <button 
                  onClick={() => confirmSocialSSO(`user_${ssoModalPlatform.toLowerCase()}@gmail.com`)}
                  className="w-full flex items-center gap-3 p-3 rounded-2xl hover:bg-slate-50 border border-slate-100 transition-colors text-left"
                >
                  ...
                  <div>
                    <p className="text-sm font-bold text-slate-900">Pembaca Reguler</p>
                    <p className="text-xs text-slate-500">Gunakan akun lain...</p>
                  </div>
                </button>
              </div>
              ...
              <div className="mt-6 pt-4 border-t border-slate-100 flex justify-end">
                <button
                  onClick={() => setSsoModalPlatform(null)}
